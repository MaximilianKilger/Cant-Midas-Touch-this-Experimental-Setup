...
class SurfaceSelector:
    ...
    def init_opencv(self):
        cv2.namedWindow('Surface Selector for %s ' % self.camera_parameter_name, cv2.WINDOW_AUTOSIZE)

        # Set mouse callbacks to extract the coordinates of clicked spots in the image
        cv2.setMouseCallback('Surface Selector for %s ' % self.camera_parameter_name, self.on_mouse_click)
    ...
    def read_config_file(self):
        ...
        if len(config.sections()) > 0:
            try:
                ...
                print('[Surface Selector Node]: ERROR in read_config_file():', e)

            print('[Surface Selector Node]: Successfully read data from config file')
        else:
            print('[Surface Selector Node]: Error reading data from config file')
    ...
    def select_surface(self, frame):
        ...
        if frame is not None:
            ...
            print('[Surface Selector Node]: Please wait until camera is ready...')
    ...
    def display_mode_calibration(self, frame):
        ...
        if len(self.last_mouse_click_coordinates) == 4:
            print('[Surface Selector Node]: Calibrated')
            self.update_table_corner_calibration(self.last_mouse_click_coordinates, frame)
            cv2.destroyAllWindows()
            return True
        cv2.imshow('Surface Selector for %s ' % self.camera_parameter_name, frame)
        return False

    def update_table_corner_calibration(self, coordinates, frame):

        self.__assign_coordinates_to_corners(coordinates)
        self.__write_new_corners_to_config_file()

        frame_height = frame.shape[0]
        frame_width = frame.shape[1]

        homography = self.get_homography(frame_height, frame_width)
        self.__save_homography_matrix_to_file(homography)

    def __assign_coordinates_to_corners(self, coordinates):
    ...
    def __write_new_corners_to_config_file(self):
        # Update corners in config file
        config = configparser.ConfigParser()
        config.read(CONFIG_FILE_NAME)
    ...
    def get_homography(self, height, width):

        pts1 = np.float32([self.table_corner_top_left,
                           self.table_corner_top_right,
                           self.table_corner_bottom_left,
                           self.table_corner_bottom_right])
    ...
    def __save_homography_matrix_to_file(self, homography):
        print('[Surface Selector Node]: Saving homography to file:', self.camera_parameter_name.split('/')[-1] + '_hom_matrix.npy')
        with open(self.camera_parameter_name.split('/')[-1] + '_hom_matrix.npy', 'wb') as f:
    ...
    def auto_calibrate(self, frame):
        ...
        if len(corners) == 4:
            ...
            self.update_table_corner_calibration(corners, frame)
            return True
        return False
...
if __name__ == "__main__":
    ...
    cap.open(camera_id, apiPreference=cv2.CAP_DSHOW)
    if not cap.isOpened():
    ...
    surf = SurfaceSelector("1.103_dev_setup")
    while True:
